...
const relativePath = route => path.join(process.cwd(), route);
...
if (require.main === module) {
  ...
  const file = fs.readFileSync(relativePath(args.path), { encoding: 'utf-8' });
  ...
  if (!args.output) {
    ...
    fs.writeFileSync(relativePath(args.output), converted);
  }
}
